...
//  Last change by 不靠谱xy on 16/4/23. 
...
//yeah\(°▽°)/
//统计：897columns 
//本程序的全部语句应用及部分算法来自《C语言程序设计》by Kochan ，Chapter1-7 与部分 Chapter12 基础技术 
#include "smilelib.h"
int main(int argc, const char * argv[])
{
    ...
    while (1)
    {
		...
		if (!strcmp(op,"fibnum"))
		{
			...
			scanf ("%d",&fibnum);
			unsigned long long int Fibonacci[fibnum];
			Fibonacci[0]=0;
			Fibonacci[1]=1;
			for (fibnum2 = 2;fibnum2 < fibnum;fibnum2++)
			{
				Fibonacci[fibnum2]=Fibonacci[fibnum2-1]+Fibonacci[fibnum2-2];
			}
			for (fibnum2 = 0;fibnum2 < fibnum;fibnum2++)
			{
				printf ("[%d]%llu\n",fibnum2,Fibonacci[fibnum2]);
			}
		}//斐波那契数列 
        if (!strcmp(op,"?"))
        {
            ...
            printf("convert:单位换算\n");
            printf("x:退出\n");
            printf("basecon:将一个十进制数转换至另一基数\n");
            printf("fibnum:生成斐波那契数列\n");
        }//帮助 
        ...
        if (   strcmp (op,"calc")
			...
			&& strcmp (op,"fibnum"))
        {
            printf ("You have enter the worong key word.If you don't konw what are the key words,please enter '?' when the program starting.\n");
        }//关键字错误
